...
function NotificationCreate() {
  ...
  const fillDemo = () => {
    const start = new Date(Date.now() + 1 * 60 * 1000); // in 10 minutes
    const expire = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000); // in 2 days

    setForm({
      title: "Scheduled Maintenance Window",
      body: "We will perform routine maintenance on servers. Services may be intermittently unavailable during this window.",
      audience: "all",
      type: "system",
      priority: 3,
      isActive: true,
      startAt: fmtLocal(start),
      expiresAt: fmtLocal(expire),
    });
  ...
  return (
    <>
      ...
      <div className="min-h-screen 2xl:mx-30 xl:mx-20 bg-gray-50 text-neutral-900">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-neutral-500">
                Fill the fields and hit Create.
              </p>
            </div>
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <p className="mt-1 text-[11px] text-neutral-500">
                  {Math.min((form.body || "").trim().length, 20)}/20 minimum
                  characters
                </p>
              </div>
            ...
            <div className="flex justify-end gap-2">
              <button
                type="button"
                onClick={fillDemo}
                className="rounded-md border border-blue-200 bg-blue-50 px-4 py-2 text-sm text-blue-700 transition hover:bg-blue-100"
                disabled={saving}
              >
                Demo
              </button>
