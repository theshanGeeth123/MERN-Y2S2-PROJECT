...
export default function UsersReport() {
  ...
  const handleDownloadPDF = () => {
    ...
    const FOOTER_Y = pageHeight - frameMargin - 10;
    const TOP_PADDING = frameMargin + 26;
    ...
    const ensureSpace = (y, required = 150) => {
      const usableBottom = pageHeight - frameMargin - bottomReserve;
    ...
    const render = (img) => {
      ...
      doc.setFontSize(12);
      doc.text(
        `Total: ${Number(total).toLocaleString()}   •   Verified: ${Number(verified).toLocaleString()} (${((verified / (total || 1)) * 100).toFixed(1)}%)   •   Unverified: ${Number(unverified).toLocaleString()} (${((unverified / (total || 1)) * 100).toFixed(1)}%)`,
        marginX,
        y
      );
      y += 22;
      ...
      autoTable(doc, {
        ...
        body: [
          ["Verified", Number(verified).toLocaleString(), total ? `${((verified/total)*100).toFixed(1)}%` : "0%"],
          ["Unverified", Number(unverified).toLocaleString(), total ? `${((unverified/total)*100).toFixed(1)}%` : "0%"],
        ],
      });
      ...
      if (byList.length) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.text(`Registrations by ${groupBy === "month" ? "Month" : "Day"}`, marginX, y);
        y += 16;
      ...
      if (ytdRows.length) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.text(`Min / Max Month (YTD ${now.getFullYear()}) — All Users`, marginX, y);
        y += 16;
        ...
        if (mm) {
          autoTable(doc, {
            ...
            body: [
              ["Minimum", monthLabel(mm.min.ym), Number(mm.min.all).toLocaleString()],
              ["Maximum", monthLabel(mm.max.ym), Number(mm.max.all).toLocaleString()],
            ],
          });
      ...
      if (doms.length) {
        ...
        const totalUsersAll = shaped.reduce((s, r) => s + (typeof r.users === "number" ? r.users : 0), 0);
        autoTable(doc, {
          ...tableBase,
          startY: y,
          head: [["Domain", "Users", "Share"]],
          body: shaped.map(r => [r.domain, Number(r.users).toLocaleString(), totalUsersAll ? `${((r.users/totalUsersAll)*100).toFixed(1)}%` : "0%"]),
        });
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-8">
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">User Analytics Dashboard</h1>
